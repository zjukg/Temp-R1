...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_rollout(self):
        ...
        elif self.config.rollout.name == 'openai':
            # OpenAI API Rollout
            from verl.workers.rollout.openai_rollout import OpenAIRollout
            from verl.workers.sharding_manager import BaseShardingManager
            
            print('[INFO] Building OpenAI API Rollout (no GPU needed)')
            rollout = OpenAIRollout(
                config=self.config.rollout,
                tokenizer=self.tokenizer
            )
            rollout_sharding_manager = BaseShardingManager()  # Dummy manager
            print('[INFO] OpenAI Rollout built successfully')
        else:
            raise NotImplementedError(f"Rollout name {self.config.rollout.name} not supported")
