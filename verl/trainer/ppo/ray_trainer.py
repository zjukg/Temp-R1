...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        reward_tensor_lst = []
        data_source_lst = []
        extra_info_field_values = {} 

        # 新增：用于保存验证结果的列表
        saved_results = []
        # is_val_only = self.config.trainer.get('val_only', False) # 注释掉，以便在训练中也能看到日志
        ...
        def _ensure_list(value):
            """Normalize non-tensor batch entries so downstream logic can index safely."""
        ...
        def _to_serializable(value):
            """Convert numpy / tuple objects to JSON-friendly python types."""
        ...
        # === 内部函数：收集结果 ===
        def collect_results(batch, reward_t):
            # if not is_val_only: return # 建议注释掉，这样训练时的验证也能看到输出
            ...
            ground_truths = []
            if isinstance(rm_data, (list, np.ndarray)) and len(rm_data) > 0:
                # 兼容不同的数据结构
                ground_truths = [item.get('ground_truth', 'N/A') if isinstance(item, dict) else str(item) for item in rm_data]
            else:
                ground_truths = ["N/A"] * len(pred_texts)
            
            # 3. Extract Questions (Prompts) and IDs
            prompts_raw = batch.non_tensor_batch.get('prompts', [])
            if isinstance(prompts_raw, torch.Tensor):
                prompts = self.tokenizer.batch_decode(prompts_raw, skip_special_tokens=True)
            else:
                prompts = _ensure_list(prompts_raw)
                if len(prompts) > 0 and isinstance(prompts[0], torch.Tensor):
                    prompts = self.tokenizer.batch_decode(prompts, skip_special_tokens=True)

            extra_infos = _ensure_list(batch.non_tensor_batch.get('extra_info', []))
            data_sources = _ensure_list(batch.non_tensor_batch.get('data_source', []))
            metadata_list = _ensure_list(batch.non_tensor_batch.get('metadata', []))
            
            ids = []
            for info in extra_infos:
                # 尝试获取各种可能的 ID 字段
                ids.append(str(info.get('id', info.get('uid', info.get('quid', 'N/A')))))
            
            # 4. Save
            for i in range(len(pred_texts)):
                saved_results.append({
                    "id": ids[i] if i < len(ids) else "N/A",
                    "question": prompts[i] if i < len(prompts) else "N/A",
                    "prediction": pred_texts[i],
                    "ground_truth": ground_truths[i] if i < len(ground_truths) else "N/A",
                    "data_source": _to_serializable(data_sources[i] if i < len(data_sources) else "unknown"),
                    "extra_info": _to_serializable(extra_infos[i] if i < len(extra_infos) else {}),
                    "metadata": _to_serializable(metadata_list[i] if i < len(metadata_list) else None),
                    #"reward": reward_t[i].sum().item()
                })
            print("\n[DEBUG] prompts type:", type(prompts))
            if len(prompts) > 0:
                print("[DEBUG] first prompt sample:", prompts[0])
            print("[DEBUG] non_tensor_batch keys:", batch.non_tensor_batch.keys())
        ...
        if not self.config.do_search:
            for test_data in self.val_dataloader:
                ...
                # ✅【修复点 1】：在这里调用收集函数
                collect_results(test_batch, reward_tensor)
                ...
                extra_info_list = test_batch.non_tensor_batch.get('extra_info', [{}]*batch_size)
                for i in range(batch_size):
                    ds = batch_data_sources[i]
                    if ds not in extra_info_field_values:
                        extra_info_field_values[ds] = {}
                    if ds.lower() == 'timelinekgqa':
                        fields_to_track = ['qlabel', 'answer_type', 'qtype']
                    else: 
                        fields_to_track = ['qlabel', 'answer_type', 'time_level', 'qtype']

                    for field in fields_to_track:
                        if field not in extra_info_field_values[ds]:
                            extra_info_field_values[ds][field] = []
                        extra_info_field_values[ds][field].append(extra_info_list[i].get(field, 'Unknown'))
        ...
        if len(saved_results) > 0:
            # 无论是否 val_only 都尝试保存，方便调试，文件名带上 global_step 防止覆盖
            step_str = f"step_{self.global_steps}" if hasattr(self, 'global_steps') else "final"
            save_path = os.path.join(self.config.trainer.default_local_dir, f'val_preds_{step_str}.json')
            try:
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                with open(save_path, 'w', encoding='utf-8') as f:
                    json.dump(saved_results, f, ensure_ascii=False, indent=2)
                print(f"✅ Validation predictions saved to: {save_path}")
            except Exception as e:
                print(f"❌ Failed to save validation predictions: {e}")
        
        # === ✅【新增】直接打印到日志 (Log Print) ===
        if len(saved_results) > 0:
            print("\n" + "="*80)
            print(f"📝 验证集采样输出预览 (Total: {len(saved_results)})")
            print("="*80)
            # 打印前 5 条详细信息，避免刷屏
            for i, item in enumerate(saved_results[:5]): 
                ...
                print("-" * 40)
            print(f"... (剩余 {len(saved_results)-5} 条已保存至文件) ...")
            print("="*80 + "\n")
        ...
        # 2. 按细分字段统计
        for ds, field_dict in extra_info_field_values.items():
            for field, values in field_dict.items():
                field_reward = {}
                for i, val in enumerate(values):
                    field_reward.setdefault(val, []).append(reward_tensor[i].item())
                for val, rewards in field_reward.items():
                    metric_dict[f'val/{field}/{val}'] = np.mean(rewards)
                    metric_dict[f'val/{field}/{val}_count'] = len(rewards)

        # 打印详细统计（控制台输出）
        print("\n" + "="*80)
        print("📊 验证统计（按 data_source 自动适配字段）")
        print("="*80)
        print(f"总样本数: {len(reward_tensor)}")
        print(f"总正确率: {np.mean(reward_tensor.numpy()):.4f}\n")
        ...
        for ds, field_dict in extra_info_field_values.items():
            ...
            for field, values in field_dict.items():
                ...
                for i, val in enumerate(values):
                    field_reward.setdefault(val, []).append(reward_tensor[i].item())
                print(f"  按 {field} 分组:")
                for val, rewards in sorted(field_reward.items()):
                    print(f"    {str(val):15s}: {np.mean(rewards):.4f} ({len(rewards):3d} samples)")
            print()
